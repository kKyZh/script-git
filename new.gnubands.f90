...
! revised by Zhang, May 1st 2019
  ...
  character(32) filename ! revised by kK later use
  ...
  if(command_argument_count()==0)then
    write(*,*)
    write(*,*)&
      'Error : You need at least one input file or with arguments'
    write(*,*)
    stop
  endif
  ...
  open(10,file='bands.dat', form='formatted',status='unknown',&
  access='sequential')
  ...
  !#############################################################
  !changed to write in a new file
  write(10,"(2a)") '# GNUBANDS: Utility for SIESTA to transform ',  &
       'bands output into Gnuplot format'
  write(10,"(a)") '#'
  write(10,"(2a)") '#                                           ',  &
       '       Emilio Artacho, Feb. 1999'
  ...
  write(10,"(2a)") '# ------------------------------------------', &
       '--------------------------------'
  if ( Fermi_shift ) then
     write(10,"(a)")  '# Energies are subtracted Ef (zero is Ef)'
  end if
  write(10,"(a,f10.4)")  '# E_F               = ', Ef
  write(10,"(a,2f10.4)") '# k_min, k_max      = ', kmin, kmax
  write(10,"(a,2f10.4)") '# E_min, E_max      = ', emin, emax
  write(10,"(a,3i6)")    '# Nbands, Nspin, Nk = ', nband, nspin, nk
  write(10,"(a,2i6)")    '# Using min_band, max_band = ', min_band, max_band
  write(10,"(a,i6)")     '# Total number of bands = ', nbands
  write(10,"(a)") '#'
  write(10,"(a)") '#        k            E'
  write(10,"(2a,/)") '# ------------------------------------------',   &
       '--------------------------------'
     ...
     do ib = min_band, max_band
        ...
        do ik = 1 , nk
           ...
           if ( emin <= e(ib,is,ik) .and. e(ib,is,ik) <= emax ) then
              add_new_line = .true.
              write(10,"(2f14.6)") k(ik), e(ib,is,ik)
           end if
        end do
        ! If the energy range has no contribution of this
        ! band, then do not add new-lines
        if ( add_new_line ) write(10,'(/)')
     enddo
  enddo
  !#############################################################
  write(6,*)''
  write(6,'(a)')'File bands.dat created...'
  write(6,*)''
  close(10)
