      ...
      do i1=2, num_ele
         do i2=1, i1-1
            ...
            do i=1, nr
               r = (i-1.0)*dr
               if (r < rst) r=rst
               psi1=Ai(ct)*exp(-alphai(ct)*(r/rei(ct)-1.0))
               psi1=psi1/(1.0+(r/rei(ct)-caii(ct))**20)
               psi2=Bi(ct)*exp(-betai(ct)*(r/rei(ct)-1.0))
               psi2=psi2/(1.0+(r/rei(ct)-ramdai(ct))**20)
               psia=psi1-psi2

               psi1=Aj(ct)*exp(-alphaj(ct)*(r/rej(ct)-1.0))
               psi1=psi1/(1.0+(r/rej(ct)-caij(ct))**20)
               psi2=Bj(ct)*exp(-betaj(ct)*(r/rej(ct)-1.0))
               psi2=psi2/(1.0+(r/rej(ct)-ramdaj(ct))**20)
               psib=psi1-psi2
