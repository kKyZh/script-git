...
C
C       revised by zhang from Apr/26/2019 slightly, will rewrite it in f90
C       and cpp
C       this is the subprogram of getldos (a bash script)
     ...
     .          species*6,squoted*6,chlab*6,owrite*1,s1*1
      ...
      character inpfilename*60 ! from argument
      character outfilename*60 ! from argument
      character tmpfilnam*60 ! .bands file for the fermi level eV
      character string_species*60 ! from argument
      character casrang*60 ! char of asrang
      integer rewrite ! from argument
      integer lfermilv ! length of fermilv
      real*8  fermilv ! fermi level in .bands
      real*8  ymax ! max value of y axis from -1 to 1
      real*8  asrang ! assigned value of range
      character cfermilv*16 ! char of fermilv
      !######## specific conditions
      !######## with bash for repeating input
      
        write(*,*)
        write(*,'(1x,2a)')'Revised from fmpdos, add change fermi ',
     .     'level to 0 eV '
        write(*,'(1x,3a)')'Initialized the intensity of DOS to 1 ',
     .     'at the range of user assigned ',
     .     '(Version --1.2 beta//Apr./28/2019//)'
        write(*,*)
              
      call get_command_argument(1,inpfilename)
      call get_command_argument(2,outfilename)
      call get_command_argument(3,string_species)
      call get_command_argument(4,casrang)
      rewrite=command_argument_count()
      ! later add check the range value by ascII method
      ! check number or character
      ! 1 .PDOS; 2 .dat; 3 species; 4 range

      if(len_trim(inpfilename)/=0)then
      tmpfilnam=
     .  ''//inpfilename(1:len_trim(inpfilename)-5)//'.bands'
        open (13,file=trim(tmpfilnam),status='old',form='formatted')
        ! .bands is 13
        read(13,*)fermilv
        write(cfermilv,'(f15.5)')fermilv
        lfermilv=len_trim(adjustl(trim(cfermilv)))
        read(cfermilv,*)fermilv
      else
        ! original fmpdos with no inputfile at first argument
      endif
...
555   continue
      ...
      !######## renewed 
      if(len_trim(outfilename)/=0)then
        outfil=trim(outfilename)
        write(*,*)trim(outfil)
        goto 556
      else
      endif
...
556   continue
      ...
      if (filexist) then
        if(rewrite.eq.4)then
          ! only auto overwrite when run getldos with 4 arguments
          owrite='y'
          goto 600
        else
        endif
...
C --- select projected DOS which are needed: -------------
  100 continue
     ...
     .            ' (enter its chemical LABEL):'
      !######## renewed 
      if(len_trim(string_species)/=0)then
        string=trim(string_species)
        write(*,*)trim(string)
        goto 557
      else
      endif
      ...
      read (5,*) string
557   continue
C     add other information for postprocessing check renewed
      write (io1,'(a)')'#'
      write (io1,'(a,f<lfermilv>.5)')
     . '#   Original fermi level : ',fermilv
      write (io1,'(a)')'#   Fermi level is shifted to 0 eV'
      write (io1,'(a)')'#   (Initialized intensity value at last line)'
...
C       1st character is a special symbol; presumably an error
      ...
      !read (5,*) nref
      ! this time use 0 for all n no read from terminal
      ! do not delete the read nref from terminal
      nref=0
      write(6,'(i0)')nref
      if (nref.ne.0) then
...
C         write (6,*) ' They match'
        ...
        goto 11
      elseif (string(2:10).eq.'position=') then    !  atom position line:
        goto 11
      elseif (string(2:3).eq.'n=') then   !  n-value:
        n = iquoted(string,nline)
        if (nref.ne.0.and.n.ne.nref) redos=.false.
        goto 11
      ...
      elseif (string(1:6).eq.'<data>') then        !  list of PDOS follows:
        !if (redos) write (io1,303) atind,n,l,m,z
        do it=1,nt
...
C --- before write, initialized intensity by ymax and save
        ! renewed get max intensity value
        ymax=0
        read(casrang,*)asrang
        do it=1,nt
            ! if no spin selected in SIESTA
            ! PDOS or LDOS nspin = 1 in .PDOS file
              !renewed to find maxy
          if(asrang==0.or.abs(asrang)>3)then
            asrang=3
          else
            asrang=abs(asrang)
          endif

          if(ene(it)<=asrang.and.ene(it)>=-asrang)then
        ! ymax is the max intensity value from the asrang 
            if(dos(it,1)>ymax)then
              ymax=dos(it,1)
            endif
          endif
                ! nspin = 1 ? only ispin = 1 obtained?
        enddo
...
C 303 format('#   Add data for atom_index =',i4,',  n,l,m,z=',4i3)
  304 format('#',/,'#    Energy',10x,'spin 1',8x,'spin2',/,'#')    
  305 format(f13.7,4f15.8)
      ! maybe for other options
